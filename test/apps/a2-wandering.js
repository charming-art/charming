...
function update(app, context) {
  ...
  const seek = force((d) => {
    const desired = cc.vecSub(d.target, a2.location).mag(d.maxSpeed);
  ...
  const target = (d) => {
    ...
    const theta = (d.wanderT += cc.random(-d.wanderC, d.wanderC)) + heading;
    const r = cc.vec(d.wanderR * Math.cos(theta), d.wanderR * Math.sin(theta));
    d.target = cc.vecAdd(circle, r);
    d.circle = circle;
  };
