...
However, when I became a front-end engineer in the field of visualization, I noticed a distinct shift in programming paradigms for both visualization grammars and front-end frameworks: from **imperative** to **declarative**. Declarative grammars can accelerate development, facilitate retargeting across platforms, and allow language-level optimizations. While P5.js, as the computational art framework for the web, is still imperative and I find myself using it less and less as I taste the benefits of declarative programming. So I began to ponder whether it was possible to design and implement a declarative grammar specifically for computational art or creative coding?

After about two year's experience developing and maintaining the new 5.0 version of [G2](https://github.com/antvis/G2), I learned the complete process of designing a new grammar. At the same time, I observed a common point between visualization and computational art: they are all **data-driven** in some level. The difference is that visualization(information visualization more specifically) is typically driven by data derived from daily life activities, while computational art is driven by data generated by algorithm or computing.

Based on this observation, I initialized the skeleton of Charming's declarative grammar and found more benefits gradually. Declarative grammar decouple specification (the what) from execution (the how). Not only does this make the code more predictable, it also does diverse the underlying rendering implementations, for instance allowing 2d primitives to be rendered by WebGL instead of Canvas to gain high performance, or rendering shapes into ASCII art style by a terminal renderer to make a wider variety of art forms possible.
